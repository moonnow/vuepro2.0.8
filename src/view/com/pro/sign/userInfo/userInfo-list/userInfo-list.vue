    ...
    <Card>
      ...
      <Collapse simple class="margin-top-10">
        <Panel>
          精准查询
          <div slot="content">
            <Form ref="userInfoVagueQueryForm" :model="userInfoVagueQuery" inline :label-width="135" class="margin-top-10">
              <FormItem label="标签：">
                <Input v-model="userInfoVagueQuery.labelName" placeholder="输入标签"></Input>
              </FormItem>
              <FormItem label="内容：">
                <Input v-model="userInfoVagueQuery.content" placeholder="输入内容"></Input>
              </FormItem>
              <FormItem label="用户编号：">
                <Input v-model="userInfoVagueQuery.userId" placeholder="输入用户编号"></Input>
              </FormItem>
...
import UserInfoQuery from '@/view/com/pro/sign/userInfo/userInfoQuery'

// @/view/com/pro/sign/userInfo/userInfo-list/userInfo-list.vue
  ...
  data () {
    return {
      columns: [
        ...
        { title: '标签', key: 'labelName', width: 200 },
      ...
      userInfoVagueQuery: new UserInfoQuery()
    }
  },
  computed: {
  ...
  methods: {
    ...
    up () {
      const route = {
        name: 'userInfo-edit',
        params: {
          userInfoId: this.selectDataRow[0].userInfoId
        },
        meta: {
          title: '编辑用户信息'
        }
    ...
    getDataSet () {
      this.spinShow = true
      this.pageShow = true
      this.pagingGetUserInfo({ rows: this.rows, page: this.page }).then(res => {
    ...
    del () {
      this.spinShow = true
      this.batchRemoveUserInfo(this.selectDataRow).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.$Message.success({ content: '用户信息信息删除成功.', duration: 3 })
          this.setSelectDataRow([])
          this.getDataSet()
        } else {
          this.$Message.error({ content: res.data.msg, duration: 6 })
    ...
    keywordSearch (keyLikeValue) {
      if (keyLikeValue !== '') {
        ...
        this.pageShow = false
        this.spinShow = true
        let userInfoQueryKeyLikeValue = new UserInfoQuery()
        userInfoQueryKeyLikeValue.labelNameAndKeyLike = '%' + keyLikeValue + '%'
        userInfoQueryKeyLikeValue.contentOrKeyLike = '%' + keyLikeValue + '%'
        userInfoQueryKeyLikeValue.userIdOrKeyLike = '%' + keyLikeValue + '%'
        this.queryUserInfo(userInfoQueryKeyLikeValue).then(res => {
          if (res.data.isSuccess && res.data.statusCode === 200) {
            this.dataSet = res.data.data
            this.count = res.data.count
            this.setSelectDataRow([])
    ...
    vagueQuery () {
      ...
      this.pageShow = false
      this.spinShow = true
      this.queryUserInfo(this.userInfoVagueQuery).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.dataSet = res.data.data
          this.count = res.data.count
          this.setSelectDataRow([])
