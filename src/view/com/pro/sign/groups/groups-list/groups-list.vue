    ...
    <Card>
      ...
      <Collapse simple class="margin-top-10">
        <Panel>
          精准查询
          <div slot="content">
            <Form ref="groupsVagueQueryForm" :model="groupsVagueQuery" inline :label-width="135" class="margin-top-10">
              <FormItem label="组标识：">
                <Input v-model="groupsVagueQuery.groupsKey" placeholder="输入组标识"></Input>
              </FormItem>
              <FormItem label="组名称：">
                <Input v-model="groupsVagueQuery.groupsName" placeholder="输入组名称"></Input>
              </FormItem>
              <FormItem label="描述：">
                <Input v-model="groupsVagueQuery.description" placeholder="输入描述"></Input>
              </FormItem>
...
import GroupsQuery from '@/view/com/pro/sign/groups/groupsQuery'

// @/view/com/pro/sign/groups/groups-list/groups-list.vue
  ...
  data () {
    return {
      columns: [
        ...
        { title: '组标识', key: 'groupsKey', width: 200 },
        { title: '组名称', key: 'groupsName', width: 200 },
      ...
      groupsVagueQuery: new GroupsQuery()
    }
  },
  computed: {
  ...
  methods: {
    ...
    up () {
      const route = {
        name: 'groups-edit',
        params: {
          groupsId: this.selectDataRow[0].groupsId
        },
        meta: {
          title: '编辑组'
        }
    ...
    getDataSet () {
      this.spinShow = true
      this.pageShow = true
      this.pagingGetGroups({ rows: this.rows, page: this.page }).then(res => {
    ...
    del () {
      this.spinShow = true
      this.batchRemoveGroups(this.selectDataRow).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
    ...
    keywordSearch (keyLikeValue) {
      if (keyLikeValue !== '') {
        ...
        this.pageShow = false
        this.spinShow = true
        let groupsQueryKeyLikeValue = new GroupsQuery()
        groupsQueryKeyLikeValue.groupsKeyAndKeyLike = '%' + keyLikeValue + '%'
        groupsQueryKeyLikeValue.groupsNameOrKeyLike = '%' + keyLikeValue + '%'
        groupsQueryKeyLikeValue.descriptionOrKeyLike = '%' + keyLikeValue + '%'
        this.queryGroups(groupsQueryKeyLikeValue).then(res => {
          if (res.data.isSuccess && res.data.statusCode === 200) {
            this.dataSet = res.data.data
            this.count = res.data.count
            this.setSelectDataRow([])
    ...
    vagueQuery () {
      ...
      this.pageShow = false
      this.spinShow = true
      this.queryGroups(this.groupsVagueQuery).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.dataSet = res.data.data
          this.count = res.data.count
          this.setSelectDataRow([])
