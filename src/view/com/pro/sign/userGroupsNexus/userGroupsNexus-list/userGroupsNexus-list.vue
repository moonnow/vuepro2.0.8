<style lang="less">
  @import './userGroupsNexus-list.less';
</style>
    ...
    <Card>
      ...
      <Collapse simple class="margin-top-10">
        <Panel>
          精准查询
          <div slot="content">
            <Form ref="userGroupsNexusVagueQueryForm" :model="userGroupsNexusVagueQuery" inline :label-width="135" class="margin-top-10">
              <FormItem label="用户编号：">
                <Input v-model="userGroupsNexusVagueQuery.userId" placeholder="输入用户编号"></Input>
              </FormItem>
              <FormItem label="组编号：">
                <Input v-model="userGroupsNexusVagueQuery.groupsId" placeholder="输入组编号"></Input>
              </FormItem>
...
import UserGroupsNexusQuery from '@/view/com/pro/sign/userGroupsNexus/userGroupsNexusQuery'

// @/view/com/pro/sign/userGroupsNexus/userGroupsNexus-list/userGroupsNexus-list.vue

export default {
  name: 'UserGroupsNexusList',
  props: {
  },
  data () {
    return {
      columns: [
        ...
        { title: '组编号', key: 'groupsId', width: 200 }
      ],
      dataSet: [],
      selectDataRow: [],
      ...
      userGroupsNexusVagueQuery: new UserGroupsNexusQuery()
    }
  },
  computed: {
  ...
  methods: {
    ...
    ...mapActions('userGroupsNexusStore', [
      'batchRemoveUserGroupsNexus',
      'pagingGetUserGroupsNexus',
      'queryUserGroupsNexus'
    ]),
    add () {
      const route = {
    ...
    up () {
      const route = {
        name: 'userGroupsNexus-edit',
        params: {
          nexusId: this.selectDataRow[0].nexusId
        },
        meta: {
          title: '编辑用户组关系'
        }
    ...
    getDataSet () {
      this.spinShow = true
      this.pageShow = true
      this.pagingGetUserGroupsNexus({ rows: this.rows, page: this.page }).then(res => {
    ...
    del () {
      this.spinShow = true
      this.batchRemoveUserGroupsNexus(this.selectDataRow).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.$Message.success({ content: '用户组关系信息删除成功.', duration: 3 })
          this.setSelectDataRow([])
          this.getDataSet()
        } else {
          this.$Message.error({ content: res.data.msg, duration: 6 })
    ...
    keywordSearch (keyLikeValue) {
      if (keyLikeValue !== '') {
        ...
        this.pageShow = false
        this.spinShow = true
        let userGroupsNexusQueryKeyLikeValue = new UserGroupsNexusQuery()
        userGroupsNexusQueryKeyLikeValue.userIdAndKeyLike = '%' + keyLikeValue + '%'
        userGroupsNexusQueryKeyLikeValue.groupsIdOrKeyLike = '%' + keyLikeValue + '%'
        this.queryUserGroupsNexus(userGroupsNexusQueryKeyLikeValue).then(res => {
          if (res.data.isSuccess && res.data.statusCode === 200) {
            this.dataSet = res.data.data
            this.count = res.data.count
            this.setSelectDataRow([])
    ...
    vagueQuery () {
      ...
      this.pageShow = false
      this.spinShow = true
      this.queryUserGroupsNexus(this.userGroupsNexusVagueQuery).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.dataSet = res.data.data
          this.count = res.data.count
          this.setSelectDataRow([])
