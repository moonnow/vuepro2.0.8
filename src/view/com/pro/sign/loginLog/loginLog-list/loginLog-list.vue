    ...
    <Card>
      ...
      <Collapse simple class="margin-top-10">
        <Panel>
          精准查询
          <div slot="content">
            <Form ref="loginLogVagueQueryForm" :model="loginLogVagueQuery" inline :label-width="135" class="margin-top-10">
              <FormItem label="账号编号：">
                <Input v-model="loginLogVagueQuery.accountId" placeholder="输入账号编号"></Input>
              </FormItem>
              <FormItem label="账号：">
                <Input v-model="loginLogVagueQuery.account" placeholder="输入账号"></Input>
              </FormItem>
              <FormItem label="登录时间：">
                <InputNumber v-model="loginLogVagueQuery.loginTimes"></InputNumber>
              </FormItem>
...
import LoginLogQuery from '@/view/com/pro/sign/loginLog/loginLogQuery'

// @/view/com/pro/sign/loginLog/loginLog-list/loginLog-list.vue
  ...
  data () {
    return {
      columns: [
        ...
        { title: '账号编号', key: 'accountId', width: 200 },
        { title: '账号', key: 'account', width: 200 },
        { title: '登录时间', key: 'loginTimes', width: 200 }
      ],
      dataSet: [],
      selectDataRow: [],
      ...
      loginLogVagueQuery: new LoginLogQuery()
    }
  },
  computed: {
  ...
  methods: {
    ...
    up () {
      const route = {
        name: 'loginLog-edit',
        params: {
          logId: this.selectDataRow[0].logId
        },
        meta: {
          title: '编辑登录日志'
        }
    ...
    getDataSet () {
      this.spinShow = true
      this.pageShow = true
      this.pagingGetLoginLog({ rows: this.rows, page: this.page }).then(res => {
    ...
    del () {
      this.spinShow = true
      this.batchRemoveLoginLog(this.selectDataRow).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
    ...
    keywordSearch (keyLikeValue) {
      if (keyLikeValue !== '') {
        ...
        this.pageShow = false
        this.spinShow = true
        let loginLogQueryKeyLikeValue = new LoginLogQuery()
        loginLogQueryKeyLikeValue.accountIdAndKeyLike = '%' + keyLikeValue + '%'
        loginLogQueryKeyLikeValue.accountOrKeyLike = '%' + keyLikeValue + '%'
        this.queryLoginLog(loginLogQueryKeyLikeValue).then(res => {
          if (res.data.isSuccess && res.data.statusCode === 200) {
            this.dataSet = res.data.data
            this.count = res.data.count
            this.setSelectDataRow([])
    ...
    vagueQuery () {
      ...
      this.pageShow = false
      this.spinShow = true
      this.queryLoginLog(this.loginLogVagueQuery).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.dataSet = res.data.data
          this.count = res.data.count
          this.setSelectDataRow([])
