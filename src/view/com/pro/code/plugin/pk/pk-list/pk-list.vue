    ...
    <Card>
      ...
      <Collapse simple class="margin-top-10">
        <Panel>
          精准查询
          <div slot="content">
            <Form ref="pkVagueQueryForm" :model="pkVagueQuery" inline :label-width="135" class="margin-top-10">
              <FormItem label="数据库表编号：">
                <Input v-model="pkVagueQuery.dtId" placeholder="输入数据库表编号"></Input>
              </FormItem>
              <FormItem label="列编号：">
                <Input v-model="pkVagueQuery.columnsId" placeholder="输入列编号"></Input>
              </FormItem>
...
import PkQuery from '@/view/com/pro/code/plugin/pk/pkQuery'
  ...
  methods: {
    ...
    up () {
      const route = {
        name: 'pk-edit',
        params: {
          pkId: this.selectDataRow[0].pkId
        },
        meta: {
          title: '编辑主键'
        }
    ...
    getDataSet () {
      this.spinShow = true
      this.pageShow = true
      this.pagingGetPk({ rows: this.rows, page: this.page }).then(res => {
    ...
    del () {
      this.spinShow = true
      this.batchRemovePk(this.selectDataRow).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
    ...
    keywordSearch (keyLikeValue) {
      if (keyLikeValue !== '') {
        ...
        this.pageShow = false
        this.spinShow = true
        let pkQueryKeyLikeValue = new PkQuery()
        pkQueryKeyLikeValue.dtIdAndKeyLike = '%' + keyLikeValue + '%'
        pkQueryKeyLikeValue.columnsIdOrKeyLike = '%' + keyLikeValue + '%'
        this.queryPk(pkQueryKeyLikeValue).then(res => {
          if (res.data.isSuccess && res.data.statusCode === 200) {
            this.dataSet = res.data.data
            this.count = res.data.count
            this.setSelectDataRow([])
    ...
    vagueQuery () {
      ...
      this.pageShow = false
      this.spinShow = true
      this.queryPk(this.pkVagueQuery).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.dataSet = res.data.data
          this.count = res.data.count
          this.setSelectDataRow([])
